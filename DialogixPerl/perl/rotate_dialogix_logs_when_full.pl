...
sub main {
	...
	if (!(-e "/usr/local/dialogix/logs/Dialogix.log.err")) {
		print "No Dialogix.err.log\n";
		#now check that java is running
		...
		print "Dialogix is running, but no data collected yet\n";
		return;
	}
...
sub restart_server {
	my $val = system("$bin/startup.sh 1>/tmp/dialogix_startup.stdout 2>/tmp/dialogix_startup.stderr");	
	if ($val != 0) {
