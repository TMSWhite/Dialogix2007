...
public class LoginTricepsServlet extends TricepsServlet {
    ...
    int processPost(HttpServletRequest req,
                    HttpServletResponse res) {
        /* validate session */
    ...
    int processAuthenticatedRequest(HttpServletRequest req,
                                    HttpServletResponse res,
    ...
    void loginPage(HttpServletRequest req,
                   HttpServletResponse res,
                   int login_err) {
    ...
    int okPage(HttpServletRequest req,
               HttpServletResponse res,
               String hiddenLoginToken) {
...
//XXX            tricepsEngine = new TricepsEngine(this.getServletConfig());
...
//XXX            tricepsEngine.doPost(req, res, out, hiddenLoginToken, restoreFile);
...
//			if (DEBUG) Logger.getLogger(LoggerName).writeln(sb.toString());
                ...
                if (conn == null) {
                    throw new Exception("Unable to connect to database");	// really need a way to report that there are database problems!
    ...
    boolean updateStatus(HttpServletRequest req,
                         String msg) {
        if (DB_TRACK_LOGINS) {
            /* 2/5/03:  Explicitly ask for session info everywhere (vs passing it as needed) */
            HttpSession session = req.getSession(false);
...
class LoginRecord {
    ...
    boolean addMapping(String varname,
                       String value) {
