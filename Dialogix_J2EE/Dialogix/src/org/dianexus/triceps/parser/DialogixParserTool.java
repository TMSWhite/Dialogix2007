...
//	protected DataSource ds = null;	// this ok as global, since used on servlet-by-servlet basis
  ...
  Connection getDBConnection() {
		try {
			...
			return DriverManager.getConnection("jdbc:mysql://localhost:3306/FirstResp?useUnicode=yes&amp;characterEncoding=UTF-8", "FirstResp", "FirstResp_pass");			
		}catch(Throwable e) {
			logger.error("",e);
			return null;
		}
	...
	boolean writeToDB(String command) {
		...
		try {
			...
			*/
			...
			if (conn == null) throw new Throwable("Unable to connect to database");
