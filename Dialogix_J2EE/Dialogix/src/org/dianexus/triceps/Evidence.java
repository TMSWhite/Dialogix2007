...
public class Evidence implements VersionIF {
    ...
    private static final Integer UNLIMITED = new Integer(-1);   // XXX CONCURRENCY RISK?

    /* Function declarations */
    private static final int DESC = 0,  ISASKED = 1,  ISNA = 2,  ISREFUSED = 3,  ISUNKNOWN = 4,  ISNOTUNDERSTOOD = 5,  ISDATE = 6,  ISANSWERED = 7,  GETDATE = 8,  GETYEAR = 9,  GETMONTH = 10,  GETMONTHNUM = 11,  GETDAY = 12,  GETWEEKDAY = 13,  GETTIME = 14,  GETHOUR = 15,  GETMINUTE = 16,  GETSECOND = 17,  NOW = 18,  STARTTIME = 19,  COUNT = 20,  ANDLIST = 21,  ORLIST = 22,  NEWDATE = 23,  NEWTIME = 24,  ISINVALID = 25,  MIN = 26,  MAX = 27,  GETDAYNUM = 28,  HASCOMMENT = 29,  GETCOMMENT = 30,  GETTYPE = 31,  ISSPECIAL = 32,  NUMANSOPTIONS = 33,  GETANSOPTION = 34,  CHARAT = 35,  COMPARETO = 36,  COMPARETOIGNORECASE = 37,  ENDSWITH = 38,  INDEXOF = 39,  LASTINDEXOF = 40,  LENGTH = 41,  STARTSWITH = 42,  SUBSTRING = 43,  TOLOWERCASE = 44,  TOUPPERCASE = 45,  TRIM = 46,  ISNUMBER = 47,  FILEEXISTS = 48,  ABS = 49,  ACOS = 50,  ASIN = 51,  ATAN = 52,  ATAN2 = 53,  CEIL = 54,  COS = 55,  EXP = 56,  FLOOR = 57,  LOG = 58,  POW = 59,  RANDOM = 60,  ROUND = 61,  SIN = 62,  SQRT = 63,  TAN = 64,  TODEGREES = 65,  TORADIANS = 66,  PI = 67,  E = 68,  FORMAT_NUMBER = 69,  PARSE_NUMBER = 70,  FORMAT_DATE = 71,  PARSE_DATE = 72,  GET_CONCEPT = 73,  GET_LOCAL_NAME = 74,  GET_EXTERNAL_NAME = 75,  GET_DEPENDENCIES = 76,  GET_ACTION_TEXT = 77,  JUMP_TO = 78,  GOTO_FIRST = 79,  JUMP_TO_FIRST_UNASKED = 80,  GOTO_PREVIOUS = 81,  ERASE_DATA = 82,  GOTO_NEXT = 83,  MEAN = 84,  STDDEV = 85,  SUSPEND_TO_FLOPPY = 86,  REGEX_MATCH = 87,  CREATE_TEMP_FILE = 88,  SAVE_DATA = 89,  EXEC = 90,  SET_STATUS_COMPLETED = 91,  SHOW_TABLE_OF_ANSWERS = 92,  PARSE_EXPR = 93,  LOAD_INSTRUMENT = 94;
    /**
...
//    static final Evidence NULL = new Evidence(null);   //  CONCURRENCY RISK?: YES
    ...
    void initReserved() {
        ...
        for (idx = 0; idx < numReserved; ++idx) {
            ...
            value.setDatum(new Datum(triceps, schedule.getReserved(idx),
                Datum.STRING), null);
        // if (logger.isLoggable(Level.FINER)) logger.log(Level.FINER,"##Evidence.initReserved(" +
    ...
    @see Datum
     */
    public Datum function(String name,
                           Vector params,
                           int line,
                           int column) {
            ...
            switch (funcNum) {
                ...
                case NEWDATE:
                    ...
                    if (params.size() == 2) {
                        ...
                        return new Datum(triceps, getParam(params.elementAt(0)).stringVal(), Datum.DATE, getParam(
                ...
                case NEWTIME:
                    if (params.size() == 2) {
                        ...
                        return new Datum(triceps, getParam(params.elementAt(0)).stringVal(), Datum.TIME, getParam(
                            params.elementAt(1)).stringVal());
                    } else if (params.size() == 3) {
                ...
                case SHOW_TABLE_OF_ANSWERS: {
                    ...
                    if (options.size() != headers.size()) {
                        setError(
                            triceps.get("function") + "showTableOfAnswers(column_variables,column_headers,rows,...) must have same number of columns for variable names and header messages",
     ...
     */
    private String functionError(int funcNum,
                                  int datumType,
                                  int index) {
        return FUNCTION_ARRAY[funcNum][FUNCTION_NAME] + " " + triceps.get("expects") + " " + Datum.getTypeName(triceps, datumType) + " " + triceps.get("at_index") + " " + index;
    }
}
