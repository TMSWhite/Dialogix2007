...
public class DialogixParser implements/*@bgen(jjtree)*/ DialogixParserTreeConstants,Serializable, VersionIF, DialogixParserConstants {/*@bgen(jjtree)*/
  ...
  static Logger logger = Logger.getLogger(DialogixParser.class);  /* XXX:  Do I need way to have triceps-specific logger? */
  ...
  */
  private void trace(String s,Datum d) {
    if (logger.isTraceEnabled()) {
      if (d == null) {
        logger.trace(s + "[null Datum]");
      }
      else {
        String varName = d.getName();

        logger.trace(((s != null) ? (s + "\t") : "") + "->\t" +
              "'" + d.stringVal(true) + "'\t" +
              d.doubleVal() + "\t" +
              d.dateVal() + "\t" +
              d.monthVal() + "\t" +
