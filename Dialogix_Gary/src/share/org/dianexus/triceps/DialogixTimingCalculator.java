...
import org.dianexus.triceps.modules.data.MysqlInstrumentSessionDataDAO;
...
//		Instrument instrument = new Instrument(Instrument.getInstrumentByName(instrumentTitle));	// FIXME - get ID of Instrument, if one exists
...
//		instrumentVersion = instrumentVersions.findByMajorVersion(new Integer(major_version).intValue());	// FIXME - check whether version exists within collection of instruments
		...
		this.getInstrumentSession();	// FIXME - how should initializatin of InstrumentSession happen?
...
//		isd = df.getInstrumentSessionDataDAO();	// FIXME - create new horizontal table
		isd = new MysqlInstrumentSessionDataDAO();	// FIXME - should be interface to data
...
//		isd.setInstrumentSessionID(this.getInstrumentSessionID().intValue());	// FIXME - how are IDs set
	...
	*/
	public void finishServerProcessing(Long timestamp){
		try {
			...
			isd.setLastAction(this.getLastAction());
			isd.setStatusMsg(this.getStatusMsg());
			isd.update();	// FIXME - persist everything at one time
			
			// Update Session State
			this.getInstrumentSession().setLastAccessTime(new Timestamp(timestamp.longValue()));
			this.getInstrumentSession().setCurrentGroup(this.getToGroupNum());
//			this.getInstrumentSession().setLastAction(this.getLastAction());	// FIXME - uses enumerated list?
			this.getInstrumentSession().setDisplayNum(this.getDisplayCount());
			this.getInstrumentSession().setLangCode(this.getLangCode());	
			this.getInstrumentSession().setStatusMsg(this.getStatusMsg());
//			this.getInstrumentSession().update();	// FIXME - persist everything at one time
...
//			this.rd.setRawData();	// FIXME - persist all ItemUsage objects at end?  So add to collection of objects to persist?
	...
	*/
	...
	private InstrumentSession initializeInstrumentSession() {
		try {
			InstrumentSession _is = new InstrumentSession();
			
			_is.setStartTime(new Timestamp(System.currentTimeMillis()));
			_is.setLastAccessTime(new Timestamp(System.currentTimeMillis()));
...
//			_is.setUserID(this.userID);	// FIXME - how are IDs set?
			_is.setInstrumentStartingGroup(this.startingStep);		
			_is.setCurrentGroup(this.startingStep);	
			_is.setDisplayNum(this.getDisplayCount());
//			_is.setLastAction("init");	// FIXME - refers to enumerated list
			_is.setStatusMsg("init");
			
//			_is.store();	//  FIXME - so that new row inserted into database - rest of interactions will be updates
			// FIXME - create new row here
			this.setInstrumentSessionID(_is.getInstrumentSessionID());
			return _is;
		} catch (Exception e) {
			logger.error("", e);
			return null;
	...
	public InstrumentSession getInstrumentSession() {
		if(this.instrumentSession==null){
			this.setInstrumentSession(initializeInstrumentSession());
		}
