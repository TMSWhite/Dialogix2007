 ...
 * UsageHitBean.java
 * 
 * Created on Oct 30, 2007, 10:48:01 AM
 * 
...
public class UsageHitBean implements VersionIF {
...
//				loadDuration =  etb.getDuration();
                ...
                latencyState = UsageHitBean.LATENCY_START;
                latencyStart = etb.getDuration();
            }
            if (etb.getActionType().equals("submit") && etb.getEventType().equals("click")) {
                // page is submitted set all states to empty
                latencyState = UsageHitBean.LATENCY_EMPTY;
                durationState = UsageHitBean.LATENCY_EMPTY;
            }
            if (!etb.getActionType().equals("submit") && etb.getEventType().equals("focus")) {

                if (latencyState == UsageHitBean.LATENCY_FINISH) {
                    latencyState = UsageHitBean.LATENCY_START;
                    latencyStart = etb.getDuration();
                }
            }
            if (etb.getEventType().equals("blur")) {
                if (durationState == UsageHitBean.DURATION_START) {
                    durationFinish = etb.getDuration();
                    durationState = UsageHitBean.DURATION_FINISH;
                }
                if (latencyState == UsageHitBean.LATENCY_START) {
                    latencyFinish = etb.getDuration();
                    latencyState = UsageHitBean.LATENCY_FINISH;
                }
            }
            if (etb.getEventType().equals("change")) {
            ...
            if (!etb.getActionType().equals("submit") && etb.getEventType().equals("click")) {
                ...
                if (durationState != UsageHitBean.DURATION_EMPTY || durationState != UsageHitBean.DURATION_FINISH) {
                    durationStart = etb.getDuration();
                    durationState = UsageHitBean.DURATION_START;
                }
            }
