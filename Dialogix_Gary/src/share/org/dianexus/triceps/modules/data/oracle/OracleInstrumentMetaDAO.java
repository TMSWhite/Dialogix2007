...
public class  OracleInstrumentMetaDAO implements InstrumentMetaDAO{
	...
	private String ORACLE_GET_METADATA_FROM_MD5 = "SELECT * FROM instrumentmeta WHERE InstrumentMD5 = ?";
	private String ORACLE_INSERT_METADATA="INSERT INTO instrumentmeta VALUES(null,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	private String ORACLE_DELETE_METADATA = "DELETE FROM instrumentmeta WHERE instrument_meta_id = ?";
	private String ORACLE_UPDATE_METADATA= "UPDATE instrumentmeta SET instrument_version_id = ? , Title = ? , Version = ? , CreationDate = ? , NumVars = ? , VarListMD5 = ? , InstrumentMD5 = ? , LanguageList = ? , NumLanguages = ? , NumInstructions = ? , NumEquations = ? , NumQuestions = ? , NumBranches = ? , NumTailorings = ?  where instrument_meta_id = ?";
	public static final String ORACLE_GET_LAST_INSERT_ID = "SELECT LAST_INSERT_ID FROM instrumentmeta";
    /** Creates a new instance of MysqlInstrumentMetaDAO */
    public  OracleInstrumentMetaDAO() {
    }
    ...
    public boolean getInstrumentMeta(java.lang.String name) {
		...
		boolean rtn = false;
		try {
			ps = con.prepareStatement(ORACLE_GET_METADATA_FROM_MD5);
			ps.clearParameters();
    ...
    public boolean setInstrumentMeta() {
		...
		boolean rtn = false;
		try {
			ps = con.prepareStatement(ORACLE_INSERT_METADATA);
			ps.clearParameters();
    ...
    public boolean updateInstrumentMeta(java.lang.String column, java.lang.String value) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_UPDATE_METADATA);
			ps.clearParameters();									
    ...
    public boolean deleteInstrumentMeta(int id) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_DELETE_METADATA);
			ps.clearParameters();
			ps.setInt(1, id);
			if (ps.executeUpdate() < 1) {
				return false;
