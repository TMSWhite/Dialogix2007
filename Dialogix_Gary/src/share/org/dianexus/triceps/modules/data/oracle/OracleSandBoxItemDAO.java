...
public class OracleSandBoxItemDAO implements SandBoxItemDAO{
	...
	private final String ORACLE_GET_SANDBOX_ITEM ="SELECT * FROM sandbox_item WHERE id= ?";
	private final String ORACLE_GET_SANDBOX_ITEMS ="SELECT * FROM sandbox_item WHERE sandbox_id= ?";
	private final String ORACLE_SET_SANDBOX_ITEM ="INSERT INTO sandbox_items SET  sandbox_id =?, instrument_id = ?, instrument_version_id =?";
	private final String ORACLE_UPDATE_SANDBOX_ITEM ="UPDATE sandbox_items SET  SET  sandbox_id =?, instrument_id = ?, instrument_version_id =?  WHERE id= ?";
	private final String ORACLE_DELETE_SANDBOX_ITEM = "DELETE sandbox_items WHERE id =?";
	...
	public boolean getSandBoxItem(int id) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_GET_SANDBOX_ITEM);
			ps.clearParameters();
			ps.setInt(1, id);
	...
	public boolean getSandBoxItems(int id) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_GET_SANDBOX_ITEMS);
			ps.clearParameters();
			ps.setInt(1, id);
	...
	public boolean setSandBoxItem() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_SET_SANDBOX_ITEM);
			ps.clearParameters();
	...
	public boolean updateSandBoxItem(int id) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_UPDATE_SANDBOX_ITEM);
			ps.clearParameters();
			ps.setInt(1, this.getSandBoxId());
			ps.setInt(2, this.getInstrumentId());
			ps.setInt(3,this.getInstrumentVersionId());
	...
	public boolean deleteSandBoxItem(int id) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_DELETE_SANDBOX_ITEM);
			ps.clearParameters();
			ps.setInt(1, id);
