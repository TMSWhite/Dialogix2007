...
public class OracleHL7OBX5DBO implements HL7OBX5DBO{
	...
	private static final String ORACLE_HL7OBX5_INSERT = "INSERT INTO mds_obx5 SET id = null, LOINC_NUM =?, SUBMITTERS_CODE=?,ANSWER_LIST_ID=?, ANSWER_ID=?, SEQUENCE_NO =?, " +
			"CODE = ?, ANSWER=?, obx5=?, obx5umls=? ";

	private static final String ORACLE_HL7OBX5_DELETE = "DELETE FROM mds_obx5 where id = ?";

	private static final String ORACLE_HL7OBX5_UPDATE = "UPDATE mds_obx5 SET LOINC_NUM =?, SUBMITTERS_CODE=?,ANSWER_LIST_ID=?,ANSWER_ID=?, SEQUENCE_NO =?, " +
			"CODE = ?, ANSWER=?, obx5=?, obx5umls=? where id =?";
	
	private static final String ORACLE_HL7OBX5_GET = "SELECT * FROM mds_obx5 WHERE id = ?";
	private static final String ORACLE_HL7OBX5_GET_ANSWER = "SELECT * FROM  mds_obx5 WHERE SUBMITTERS_CODE = ? AND CODE = ?";
	...
	public boolean deleteHL7OBX5(int id) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_HL7OBX5_DELETE);
			ps.clearParameters();
			ps.setInt(1,id);
			ps.execute();
	...
	public boolean updateHL7OBX5() {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_HL7OBX5_UPDATE);
			ps.clearParameters();
	...
	public boolean getHL7OBX5(int id) {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_HL7OBX5_GET);
			ps.clearParameters();
			ps.setInt(1,id);
	...
	public boolean getHL7OBX5(String code, String answer) {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_HL7OBX5_GET_ANSWER);
			ps.clearParameters();
			ps.setString(1,code);
	...
	public boolean setHL7OBX5() {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_HL7OBX5_INSERT);
			ps.clearParameters();
