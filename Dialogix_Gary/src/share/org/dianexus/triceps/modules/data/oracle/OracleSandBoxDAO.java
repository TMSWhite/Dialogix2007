...
public class OracleSandBoxDAO implements SandBoxDAO{
	...
	private final String ORACLE_GET_SANDBOX ="SELECT * FROM sandbox WHERE id= ?";
	private final String ORACLE_SET_SANDBOX ="INSERT INTO sandbox SET  name = ?, path = ?, url =? , port =?";
	private final String ORACLE_UPDATE_SANDBOX ="UPDATE sandbox SET  SET  name = ?, path = ?, url =? , port =? WHERE id= ?";
	private final String ORACLE_DELETE_SANDBOX = "DELETE sandbox WHERE id =?";
	...
	public boolean getSandBox(int sandBoxId) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_GET_SANDBOX);
			ps.clearParameters();
	...
	public boolean setSandBox() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_SET_SANDBOX);
			ps.clearParameters();
	...
	public boolean deleteSandBox(int sandBoxId) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_DELETE_SANDBOX);
			ps.clearParameters();
	...
	public boolean updateSandBox(int sandBoxId) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_UPDATE_SANDBOX);
			ps.clearParameters();
			ps.setString(1, this.getName());
			ps.setString(2, this.getApplicationPath());
			ps.setString(3,this.getURL());
