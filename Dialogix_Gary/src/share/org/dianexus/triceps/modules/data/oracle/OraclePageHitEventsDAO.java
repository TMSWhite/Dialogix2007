 ...
 * 
 */
public class OraclePageHitEventsDAO implements PageHitEventsDAO {
	...
	private static final String ORACLE_PAGE_HIT_EVENTS_NEW = "INSERT INTO pagehitevents SET pageHitID = ? , "
			+ " varName = ? , actionType = ? , eventType = ? ,"
			+ " timestamp = ? , duration = ? , value1 = ? , value2 = ?";

	private static final String ORACLE_PAGE_HIT_EVENTS_DELETE = "DELETE FROM pagehitevents where pageHitEventsID = ?";

	private static final String ORACLE_PAGE_HIT_EVENTS_UPDATE = "UPDATE pagehitevents SET pageHitID = ? , "
			+ " varName = ? , actionType = ? , eventType = ? ,"
			+ " timestamp = ? , duration = ? , value1 = ? , value2 = ? WHERE pageHitEventsID = ?";

	private static final String ORACLE_PAGE_HIT_EVENTS_GET = "SELECT count(*),pageHitEventsID FROM  pagehitevents WHERE pageHitID = ?";

	private static final String ORACLE_PAGE_HIT_EVENT_GET = "SELECT * FROM pagehitevents WHERE pageHitEventsID = ?";
	...
	public boolean setPageHitEvent() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_PAGE_HIT_EVENTS_NEW);
			ps.clearParameters();
	...
	public boolean getPageHitEvents(int pageHitEventId) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_PAGE_HIT_EVENT_GET);
			ps.clearParameters();
	...
	public int[] getPageHitEventIds(int pageHitId) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_PAGE_HIT_EVENTS_GET);
			ps.clearParameters();
			ps.setInt(1, pageHitEventId);
			rs = ps.executeQuery();
			rs.next();
...
//		 insert a page hit event into the table
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_PAGE_HIT_EVENTS_UPDATE);
			ps.clearParameters();
	...
	public boolean updatePageHitEventColumn(String column) {
		String ORACLE_PAGE_HIT_EVENTS_UPDATE = "UPDATE pagehitevents SET "+column+" = ?  WHERE pageHitEventsID = ?";
//		 get the connection
		Connection con = DialogixOracleDAOFactory.createConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public boolean deletePageHitEvent() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_PAGE_HIT_EVENTS_DELETE);
			ps.clearParameters();
