...
public class OracleSandBoxUserDAO implements SandBoxUserDAO {
	private int id;
	private int user_id;
	...
	private final String ORACLE_GET_USER_SANDBOXES ="SELECT * FROM sandbox_user WHERE user_id= ?";
	private final String ORACLE_GET_SANDBOX_USERS ="SELECT * FROM sandbox_user WHERE sandbox_id= ?";
	private final String ORACLE_GET_SANDBOX_ROLE_USERS ="SELECT * FROM sandbox_user WHERE sandbox_id= ? AMD role_id=?";
	private final String ORACLE_SET_SANDBOX_USER ="INSERT INTO sandbox_user SET  sandbox_id = ?, user_id = ?, role_id =?";
	private final String ORACLE_DELETE_SANDBOX_USER = "DELETE sandbox_user WHERE id =?";
	private final String ORACLE_GET_LAST_INSERT_ID = "SELECT LAST_INSERT_ID()";
	...
	public boolean getSandBoxUsers(int sandbox_id) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_GET_SANDBOX_USERS);
			ps.clearParameters();
	...
	public boolean getSandBoxUsers(int sandbox_id, int role_id) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_GET_SANDBOX_ROLE_USERS);
			ps.clearParameters();
			ps.setInt(1, sandbox_id);
			ps.setInt(2,role_id);  
	...
	public boolean getSandUserSandboxes(int user_id) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_GET_USER_SANDBOXES);
			ps.clearParameters();
			ps.setInt(1, user_id);
			rs = ps.executeQuery();
			while(rs.next()){
	...
	public boolean setSandBoxUser() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_SET_SANDBOX_USER);
			ps.clearParameters();
	...
	public boolean deleteSandBoxUser(int id) {
		boolean rtn =  false;
		Connection con = DialogixOracleDAOFactory.createConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_DELETE_SANDBOX_USER);
			ps.clearParameters();
