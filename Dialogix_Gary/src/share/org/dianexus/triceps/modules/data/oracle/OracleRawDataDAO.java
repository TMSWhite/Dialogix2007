...
/**
 * OracleRawDataDAO is an Oracle implementation of the Interface RawDataDAO This
 ...
 * 
 */
public class OracleRawDataDAO implements RawDataDAO {
	...
	// sql queries
	private static final String ORACLE_RAW_DATA_NEW = "INSERT INTO rawdata SET InstrumentName = ?,"
	...
	private static final String ORACLE_RAW_DATA_UPDATE = "UPDATE rawdata SET InstrumentName = ?,"
			+ "InstanceName = ?, VarName = ?, VarNum = ?,"
			+ "GroupNum = ?, DisplayNum = ?, LangNum = ?,"
			+ "WhenAsMS = ?, TimeStamp = ?, AnswerType = ? ,"
			+ "Answer = ?, QuestionAsAsked = ?, Comment = ? ,"
	...
	private static final String ORACLE_RAW_DATA_GET = " SELECT *  FROM rawdata WHERE RawDataID = ?";

	private static final String ORACLE_GET_LAST_INSERT_ID = "SELECT LAST_INSERT_ID()";

	private static final String ORACLE_RAW_DATA_DELETE = "DELETE FROM rawdata  WHERE RawDataID = ?";
	...
	public boolean updateRawData() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_RAW_DATA_UPDATE);
			ps.clearParameters();
	...
	public boolean setRawData() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_RAW_DATA_NEW);
			ps.clearParameters();
			ps.setString(1, instrumentName);
			ps.setString(2, instanceName);
			ps.setString(3, varName);
	...
	public boolean getRawData() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_RAW_DATA_GET);
			ps.clearParameters();
	...
	public boolean deleteRawData() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_RAW_DATA_DELETE);
			ps.clearParameters();
			ps.setInt(1, this.getRawDataId());
			if (ps.executeUpdate() < 1) {
				return false;
