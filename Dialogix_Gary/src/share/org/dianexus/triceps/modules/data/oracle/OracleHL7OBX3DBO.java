...
public class OracleHL7OBX3DBO implements HL7OBX3DBO{
	...
	private static final String ORACLE_HL7OBX3_INSERT = "INSERT INTO mds_obx3 SET id = null, SUBMITTERS_CODE = ?,LOINC_NUM =?, obx3=?, obx3umls=? ";
	private static final String ORACLE_HL7OBX3_DELETE = "DELETE FROM mds_obx3 where id = ?";

	private static final String ORACLE_HL7OBX3_UPDATE = "UPDATE mds_obx3 SET SUBMITTERS_CODE = ?, LOINC_NUM =?, obx3=?, obx3umls=?  where id =?";
	
	private static final String ORACLE_HL7OBX3_GET = "SELECT * FROM mds_obx3 WHERE id = ?";
	private static final String  ORACLE_HL7OBX3_GET_SUBMITTER_CODE ="SELECT * FROM mds_obx3 WHERE SUBMITTERS_CODE = ?";
	...
	public boolean deleteHL7OBX3(int id) {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_HL7OBX3_DELETE);
			ps.clearParameters();
			ps.setInt(1,id);
			ps.execute();
			ret = true;
	...
	public boolean updateHL7OBX3() {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_HL7OBX3_UPDATE);
			ps.clearParameters();
	...
	public boolean getHL7OBX3(int id) {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_HL7OBX3_GET);
			ps.clearParameters();
			ps.setInt(1, id);
	...
	public boolean getHL7OBX3(String  code) {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_HL7OBX3_GET_SUBMITTER_CODE);
			ps.clearParameters();
			ps.setString(1, code);
	...
	public boolean setHL7OBX3() {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_HL7OBX3_INSERT);
			ps.clearParameters();
