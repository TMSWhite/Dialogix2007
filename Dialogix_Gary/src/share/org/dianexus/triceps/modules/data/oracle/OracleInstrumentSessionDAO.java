 ...
 *
 */
public class OracleInstrumentSessionDAO implements InstrumentSessionDAO {
	...
	private static final String ORACLE_INSTRUMENT_SESSION_NEW = "INSERT INTO instrument_session SET start_time = ? , "
			+ " end_time = ? , instrument_id = ? , user_id = ? ,"
			+ " first_group = ? , last_group = ? , last_action = ? ,"
			+ " last_access = ? , statusMsg = ? ";

	private static final String ORACLE_INSTRUMENT_SESSION_DELETE = "DELETE FROM instrument_session WHERE instrument_session_id = ?";

	private static final String ORACLE_INSTRUMENT_SESSION_UPDATE = "UPDATE instrument_session SET start_time = ? , "
			+ " end_time = ? , instrument_id = ? , user_id = ? ,"
			+ " first_group = ? , last_group = ? , last_action = ? ,"
			+ " last_access = ? , statusMsg = ? WHERE instrument_session_id = ?"; 

	private static final String ORACLE_INSTRUMENT_SESSION_GET = "SELECT * FROM instrument_session WHERE instrument_session_id = ?";
	...
	public boolean setInstrumentSession() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_INSTRUMENT_SESSION_NEW);
			ps.clearParameters();
	...
	public boolean getInstrumentSession(int instrumentSessionId) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_INSTRUMENT_SESSION_GET);
			ps.clearParameters();
	...
	public boolean updateInstrumentSessionColumn(String column) {
		String ORACLE_INSTRUMENT_SESSION_COLUMN_UPDATE = "UPDATE instrument_session SET "+column+" = ?  WHERE instrument_session_id = ?";
//		 get the connection
		Connection con = DialogixOracleDAOFactory.createConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_INSTRUMENT_SESSION_COLUMN_UPDATE);
			ps.clearParameters();
	...
	public boolean updateInstrumentSession() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_INSTRUMENT_SESSION_UPDATE);
			ps.clearParameters();
			ps.setTimestamp(1, this.startTime);
			ps.setTimestamp(2, this.endTime);
			ps.setInt(3, this.instrumentVersionId);
	...
	public boolean deleteInstrumentSession() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_INSTRUMENT_SESSION_DELETE);
			ps.clearParameters();
