...
public class OracleInstrumentVersionDAO implements InstrumentVersionDAO {

	private static final String ORACLE_GET_LAST_INSERT_ID = "SELECT LAST_INSERT_ID()";

	private static final String ORACLE_INSTRUMENT_VERSION_NEW = "INSERT INTO instrument_version SET instrument_id = ? , "
	...
	private static final String ORACLE_INSTRUMENT_VERSION_GET_1 = "SELECT * FROM instrument_version WHERE instrument_version_id = ? ";
	private static final String ORACLE_INSTRUMENT_VERSION_GET_2 = "SELECT * FROM instrument_version WHERE instrument_id = ? AND major_version = ? AND minor_version =?";
	private static final String ORACLE_INSTRUMENT_VERSION_MAJOR_EXISTS = "SELECT * FROM instrument_version WHERE instrument_id = ? AND major_version = ? ";
	...
	public boolean getInstrumentVersion(int _id ) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_INSTRUMENT_VERSION_GET_1);
			ps.clearParameters();
			ps.setInt(1, _id);
			rs = ps.executeQuery();
			if (rs.next()) {
	...
	public boolean getInstrumentVersion(int _id, int major, int minor) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_INSTRUMENT_VERSION_GET_2);
			ps.clearParameters();
			ps.setInt(1, _id);
			ps.setInt(2, major);
			ps.setInt(3, minor);
	...
	public boolean InstrumentMajorVersionExists(int _id, int major ) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_INSTRUMENT_VERSION_MAJOR_EXISTS);
			ps.clearParameters();
			ps.setInt(1, _id);
			ps.setInt(2, major);
