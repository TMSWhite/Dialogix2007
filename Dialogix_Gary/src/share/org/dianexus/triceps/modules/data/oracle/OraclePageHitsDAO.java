...
public class OraclePageHitsDAO implements PageHitsDAO {
	...
	private static final String ORACLE_PAGEHITS_NEW = "INSERT INTO pagehits SET instrument_session_id = ? , "
	...
	private static final String ORACLE_PAGEHITS_DELETE = "DELETE FROM pagehits WHERE pageHitId = ?";

	private static final String ORACLE_PAGEHITS_UPDATE = "UPDATE pagehits SET instrument_session_id = ? , "
			+ " timeStamp = ? , accessCount = ? , groupNum = ? ,"
			+ " displayNum = ? , lastAction = ? , statusMsg = ? ,"
			+ " totalDuration = ? , serverDuration = ? , loadDuration = ? ,"
			+ " networkDuration = ? , pageVacillation = ?  WHERE pageHitId = ?";

	private static final String ORACLE_PAGEHITS_GET = "SELECT * FROM pagehits WHERE pageHitId = ?";
	...
	public boolean setPageHit() {
		...
		ResultSet rs =null;
		try {
			ps = con.prepareStatement(ORACLE_PAGEHITS_NEW);
			ps.clearParameters();
	...
	public boolean getPageHit(int pageHitId) {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(ORACLE_PAGEHITS_GET);
			ps.clearParameters();
	...
	public boolean updatePageHitColumn(String column) {
		...
		String ORACLE_PAGEHITS_UPDATE = "UPDATE pagehits SET " + column
				+ " = ?   WHERE pageHitId = ?";
		try {
			ps = con.prepareStatement(ORACLE_PAGEHITS_UPDATE);
			ps.clearParameters();
	...
	public boolean deletePageHit() {
		...
		PreparedStatement ps = null;
		try {
			ps = con.prepareStatement(ORACLE_PAGEHITS_DELETE);
			ps.clearParameters();
			ps.setInt(1, this.getPageHitId());
			if(ps.executeUpdate()<1){
				return false;
