...
public class MysqlSandBoxUserDAO implements SandBoxUserDAO {
  static Logger logger = Logger.getLogger(MysqlSandBoxUserDAO.class);
	...
	private int role_id;
	private ArrayList sandboxes = new ArrayList();
	private ArrayList sandboxUsers=new ArrayList();
	...
	private final String SQL_DELETE_SANDBOX_USER = "DELETE from sandbox_user WHERE id =?";
	...
	public boolean updateSandBoxUser() {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement(SQL_UPDATE_SANDBOX_USER);
			ps.clearParameters();
			ps.setInt(1, this.getSandBoxId());
			ps.setInt(2, this.getSandBoxUserRoleId());
			ps.setInt(3, this.getSandBoxUserRoleId());
