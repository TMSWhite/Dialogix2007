...
public class OracleUserDAO implements UserDAO {
	...
	private static final String ORACLE_USERS_INSERT = "INSERT INTO users SET id = null, user_name = ?, password = ?, first_name = ?, last_name = ?, phone =?, email = ? ";
	private static final String ORACLE_USERS_DELETE = "DELETE FROM users where id = ?";

	private static final String ORACLE_USERS_UPDATE = "UPDATE users SET user_name = ?, password = ?, first_name = ?, last_name = ?, phone =?, email = ?   where id =?";
	
	private static final String ORACLE_USERS_GET = "SELECT * FROM users WHERE id = ?";
	
	private static final String ORACLE_USERS_LOGIN_GET = "SELECT * FROM users WHERE user_name = ? AND password = ?";
	...
	public boolean deleteUser(int id) {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_USERS_DELETE);
			ps.clearParameters();
			ps.setInt(1,id);
			ps.execute();
			ret = true;
	...
	public boolean getUser(int id) {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_USERS_GET);
			ps.clearParameters();
			ps.setInt(1, id);
	...
	public boolean getUser(String userName, String password) {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_USERS_LOGIN_GET);
			ps.clearParameters();
	...
	public boolean setUser() {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_USERS_INSERT);
			ps.clearParameters();
	...
	public boolean updateUser(int id) {
		...
		boolean ret = false;
		try {
			ps = con.prepareStatement(ORACLE_USERS_UPDATE);
			ps.clearParameters();
